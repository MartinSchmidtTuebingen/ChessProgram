...
void Test::TestChessGameFunctions() {
  ...
  p->CreatePiece(king,blackNumber,2,5);
  p->CreatePiece(queen,whiteNumber,5,4);
  p->WriteOutPosition();
  game = new ChessGame(0x0,0x0,p);
  cout << "Memory at begin: " << getCurrentMemoryUbuntu() << endl;
  EvalMoveList* eml = game->GetEvalMoveList(4);
  cout << "Memory at end " << getCurrentMemoryUbuntu() << endl;
